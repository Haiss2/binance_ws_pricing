...
func SavePrices(db *mongo.Database, prices []*binance.WsMarketStatEvent) error {
	...
	for _, p := range prices {
		vals = append(vals, bson.D{{"price", toFloat(p.LastPrice)}, {"symbol", p.Symbol}, {"timestamp", p.Time}})
	}

	res, err := db.Collection("prices").InsertMany(context.Background(), vals, opts)
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	db := client.Database("all_pricing")
	testWs(db)
}
