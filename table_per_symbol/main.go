...
func SavePrices(db *sqlx.DB, prices []*binance.WsMarketStatEvent) error {
	fmt.Println("storing data ... ", len(prices))
	for _, p := range prices {
		go func(p *binance.WsMarketStatEvent, db *sqlx.DB) {
			query := fmt.Sprintf(`INSERT INTO "%v" (price, timestamp) VALUES ($1, $2)`, p.Symbol)
			_, err := db.Exec(query, p.LastPrice, p.Time)
			if err != nil {
				fmt.Println("Err while log", p.Symbol, err)
			}
		}(p, db)
...
func getSupported(prices []*binance.WsMarketStatEvent) []*binance.WsMarketStatEvent {
	result := make([]*binance.WsMarketStatEvent, 0)
	for _, p := range prices {
		_, ok := supportedSymbolsMap[p.Symbol]
		if ok {
...
func testWs(db *sqlx.DB) {
	wsDepthHandler := func(events binance.WsAllMarketsStatEvent) {
		SavePrices(db, fake24TimesData(getSupported(events)))
	}
